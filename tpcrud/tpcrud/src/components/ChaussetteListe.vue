  ...
  <div class="list row">
    ...
    <div class="col-md-6">
      ...
      <ul class="list-group">
        <li class="list-group-item"
          ...
          v-for="(Chaussette, index) in Chaussettes"
          :key="index"
    ...
    <div class="col-md-6">
      <div v-if="currentChaussette">
        ...
        <div>
          <label><strong>Description:</strong></label> {{ currentChaussette.description }}
        </div>
        <div>
          <label><strong>Status:</strong></label> {{ currentChaussette.published ? "Published" : "Pending" }}
        </div>

        <router-link :to="'/Chaussette/' + currentChaussette.id" class="m-3 btn btn-primary">Editer</router-link>
      </div>
      <div v-else>
        <br />
        <p>Humm pas de chaussettes sélectionnées...</p>
      </div>
    </div>
  </div>
</template>
